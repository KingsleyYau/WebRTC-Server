...
typedef enum {
	PROP_ID = 1, PROP_AGENT, PROP_STREAM,
} NiceComponentProperty;

static void
nice_component_constructed(GObject *obj);
static void
nice_component_get_property(GObject *obj, guint property_id, GValue *value,
		GParamSpec *pspec);
static void
nice_component_set_property(GObject *obj, guint property_id,
...
static gboolean epoll_source_dispatch(GSource *source, GSourceFunc callback,
		gpointer user_data) {
	IOSource *isrc = (IOSource *) source;
//	nice_debug ("Source %p(FD %d): epoll_source_dispatch, isrc->condition %d", source, isrc->fd, isrc->condition);

	if ( g_source_is_destroyed (g_main_current_source ()) ) {
		nice_debug ("Source %p destroyed before dispatch", g_main_current_source ());
...
static void epoll_source_finalize(GSource *source) {
	...
	g_mutex_lock(&sources_mutex);
	nice_debug("Source %p(FD %d): Finalize", source, isrc->fd);
	g_object_unref(isrc->gsocket);
	g_mutex_unlock(&sources_mutex);
}

static GSourceFuncs epoll_source_funcs = { epoll_source_prepare,
		epoll_source_check, epoll_source_dispatch, epoll_source_finalize };
...
GSource* nice_epoll_create_source(SocketSource *socket_source) {
	...
	if (epoll_ctl(nice_epoll_fd(), EPOLL_CTL_ADD, fd, &ev) == -1) {
		nice_debug("Socket %p(FD %d): Add to epoll(FD %d) error",
				socket_source->socket, fd, nice_epoll_fd());
		return NULL;
	}
...
nice_component_new(guint id, NiceAgent *agent, NiceStream *stream) {
	return g_object_new(NICE_TYPE_COMPONENT, "id", id, "agent", agent, "stream",
			stream,
			NULL);
}
	...
	for (i = cmp->local_candidates; i;) {
		...
		if (candidate == cmp->selected_pair.local) {
			if (cmp->turn_candidate) {
				relay_candidates = g_slist_append(relay_candidates,
						cmp->turn_candidate);
			}
...
static void nice_component_clear_selected_pair(NiceComponent *component) {
 ...
 * state. */
void nice_component_close(NiceAgent *agent, NiceComponent *cmp) {
	IOCallbackData *data;
	GOutputVector *vec;
	IncomingCheck *c;
	...
	if (cmp->restart_candidate)
		nice_candidate_free(cmp->restart_candidate), cmp->restart_candidate =
		NULL;
...
gboolean nice_component_find_pair(NiceComponent *cmp, NiceAgent *agent,
	...
	for (i = cmp->remote_candidates; i; i = i->next) {
		NiceCandidate *candidate = i->data;
		if (strncmp(candidate->foundation, rfoundation,
		NICE_CANDIDATE_MAX_FOUNDATION) == 0) {
	...
	if (result.local && result.remote) {
		result.priority = agent_candidate_pair_priority(agent, result.local,
				result.remote);
		if (pair)
			*pair = result;
		return TRUE;
 ...
 * session.
 */
void nice_component_restart(NiceComponent *cmp) {
	GSList *i;
	IncomingCheck *c;
	...
	g_slist_free(cmp->remote_candidates), cmp->remote_candidates = NULL;
...
nice_component_set_selected_remote_candidate(NiceComponent *component,
	...
	for (item = component->local_candidates; item; item = g_slist_next(item)) {
		...
		if (tmp->transport != conn_check_match_transport(candidate->transport)
				|| tmp->addr.s.addr.sa_family
						!= candidate->addr.s.addr.sa_family
	...
	if (!remote) {
		...
		component->remote_candidates = g_slist_append(
				component->remote_candidates, remote);
...
static gint _find_socket_source(gconstpointer a, gconstpointer b) {
...
void nice_component_attach_socket(NiceComponent *component,
		NiceSocket *nicesock) {
	GSList *l;
	...
	if (l != NULL) {
		...
		component->socket_sources = g_slist_prepend(component->socket_sources,
				socket_source);
 ...
 * Must *not* take the agent lock, since it’s called from within
 * nice_component_set_io_context(), which holds the Component’s I/O lock. */
static void nice_component_reattach_all_sockets(NiceComponent *component) {
	GSList *i;
...
static void nice_component_detach_socket(NiceComponent *component,
	...
	/**
	 * Add by Max 2020/01/29
	 * Call stack will can not unreference socket_sources->source
	 * Leave it unreference when nice_component_free_socket_sources
	 * g_source_attach
	 *   ->component_io_cb
...
void nice_component_set_io_context(NiceComponent *component,
		GMainContext *context) {
...
gboolean nice_component_has_io_callback(NiceComponent *component) {
...
static gboolean emit_io_callback_cb(gpointer user_data) {
	...
	while (TRUE) {
		...
		io_callback(agent, stream_id, component_id,
				...
				(gchar *) data->buf + data->offset, io_user_data);
	...
	done: g_object_unref(agent);

	return G_SOURCE_REMOVE;
}

/* This must be called with the agent lock *held*. */
void nice_component_emit_io_callback(NiceAgent *agent, NiceComponent *component,
	...
	if (g_main_context_is_owner(component->ctx)) {
		...
		agent_unlock_and_emit(agent);
		io_callback(agent, stream_id, component_id, buf_len, (gchar *) buf,
				io_user_data);
		agent_lock(agent);
	} else {
		IOCallbackData *data;
...
static void nice_component_class_init(NiceComponentClass *klass) {
	...
	g_object_class_install_property(object_class, PROP_ID,
			g_param_spec_uint("id", "ID",
					"The unique numeric ID of the component.", 1, G_MAXUINT, 1,
	...
	g_object_class_install_property(object_class, PROP_AGENT,
			g_param_spec_object("agent", "Agent",
					"The NiceAgent this component belongs to.",
					NICE_TYPE_AGENT,
					G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
...
static void source_set_dummy_callback(GSource *source) {
	g_source_set_callback(source, dummy_callback, NULL, NULL);
}

static void nice_component_init(NiceComponent *component) {
	...
	component->stop_cancellable_source = g_cancellable_source_new(
			component->stop_cancellable);
...
static void nice_component_constructed(GObject *obj) {
	NiceComponent *component;
	NiceAgent *agent;
...
static void nice_component_get_property(GObject *obj, guint property_id,
...
static void nice_component_set_property(GObject *obj, guint property_id,
		const GValue *value, GParamSpec *pspec) {
	NiceComponent *component;
 ...
 * NiceIOStreams. */
static void nice_component_finalize(GObject *obj) {
	NiceComponent *cmp;
	 ...
	 */
	nice_debug("Compenent %p: Destroyed", cmp);
...
static gboolean component_source_prepare(GSource *source, gint *timeout_) {
	...
	for (parentl = component->socket_sources; parentl; parentl =
			parentl->next) {
		...
		child_socket_source->source = g_socket_create_source(
				child_socket_source->socket->fileno, G_IO_IN,
				NULL);
		...
		component_source->socket_sources = g_slist_prepend(
				component_source->socket_sources, child_socket_source);
	}
	...
	for (childl = component_source->socket_sources; childl;) {
		...
		if (!parentl) {
			...
			component_source->socket_sources = g_slist_delete_link(
					component_source->socket_sources, childl);
		}
...
static gboolean component_source_dispatch(GSource *source, GSourceFunc callback,
		gpointer user_data) {
	ComponentSource *component_source = (ComponentSource *) source;
...
static void component_source_finalize(GSource *source) {
	...
	g_slist_free_full(component_source->socket_sources,
			free_child_socket_source);
...
static gboolean component_source_closure_callback(GObject *pollable_stream,
		gpointer user_data) {
...
static GSourceFuncs component_source_funcs = { component_source_prepare,
NULL, /* check */
component_source_dispatch, component_source_finalize, (GSourceFunc) G_CALLBACK(
...
nice_component_input_source_new(NiceAgent *agent, guint stream_id,
	...
	component_source = (ComponentSource *) g_source_new(&component_source_funcs,
			sizeof(ComponentSource));
...
void nice_component_add_valid_candidate(NiceAgent *agent,
	...
	if (nice_debug_is_enabled()) {
		...
		nice_debug("Agent %p:  %d:%d Adding valid source"
				" candidate: %s:%d trans: %d", agent, candidate->stream_id,
				candidate->component_id, str,
				nice_address_get_port(&candidate->addr), candidate->transport);
	}

	component->valid_candidates = g_list_prepend(component->valid_candidates,
