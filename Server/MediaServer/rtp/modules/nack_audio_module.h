...
#ifndef RTP_MODULES_NACK_AUDIO_MODULE_H_
#define RTP_MODULES_NACK_AUDIO_MODULE_H_
...
public:
	...
	static const size_t kNackListSizeLimit = 500;  // 10 seconds for 20 ms frame
												   // packets.
	// Factory method.
	static NackAudioModule* Create(Clock* clock, int nack_threshold_packets);

	// Constructor.
	explicit NackAudioModule(Clock* clock, int nack_threshold_packets);
...
private:
	struct NackElement {
		NackElement(int64_t initial_time_to_play_ms, uint32_t initial_timestamp,
				bool missing, int64_t initial_sent_at_time) :
				time_to_play_ms(initial_time_to_play_ms), estimated_timestamp(
						initial_timestamp), is_missing(missing), sent_at_time(initial_sent_at_time) {
		}
	...
	public:
		bool operator()(uint16_t sequence_number_old,
			...
			return IsNewerSequenceNumber(sequence_number_new,
					sequence_number_old);
		}
	};
	...
	void UpdateEstimatedPlayoutTime();
	...
	uint16_t sequence_num_first_received_rtp_;
	uint32_t timestamp_first_received_rtp_;
	...
	Timestamp time_first_received_rtp_;
	Timestamp time_last_check_rtp_;
