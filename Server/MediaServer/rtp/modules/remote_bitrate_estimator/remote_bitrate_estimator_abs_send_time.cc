...
uint32_t ConvertMsTo24Bits(int64_t time_ms) {
	uint32_t time_24_bits =
			static_cast<uint32_t>(((static_cast<uint64_t>(time_ms)
					<< RTPHeaderExtension::kAbsSendTimeFraction) + 500) / 1000)
					& 0x00FFFFFF;
	return time_24_bits;
}

RemoteBitrateEstimatorAbsSendTime::~RemoteBitrateEstimatorAbsSendTime() = default;

void RemoteBitrateEstimatorAbsSendTime::Reset() {
...
bool RemoteBitrateEstimatorAbsSendTime::IsWithinClusterBounds(int send_delta_ms,
...
RemoteBitrateEstimatorAbsSendTime::RemoteBitrateEstimatorAbsSendTime(
		...
		clock_(clock), observer_(observer), inter_arrival_(), estimator_(), detector_(
				&field_trials_), incoming_bitrate_(kBitrateWindowMs, 8000), incoming_bitrate_initialized_(
				false), total_probes_received_(0), first_packet_time_ms_(-1), last_update_ms_(
				-1), uma_recorded_(false), remote_rate_(&field_trials_) {
...
std::list<Cluster>::const_iterator RemoteBitrateEstimatorAbsSendTime::FindBestProbe(
	...
	for (std::list<Cluster>::const_iterator it = clusters.begin();
		...
		if (it->num_above_min_delta > it->count / 2
			...
			int probe_bitrate_bps = std::min(it->GetSendBitrateBps(),
					it->GetRecvBitrateBps());
...
RemoteBitrateEstimatorAbsSendTime::ProbeResult RemoteBitrateEstimatorAbsSendTime::ProcessClusters(
	...
	if (best_it != clusters.end()) {
		int probe_bitrate_bps = std::min(best_it->GetSendBitrateBps(),
...
//					<< best_it->GetRecvBitrateBps() << " bps. Mean send delta: "
//					<< best_it->send_mean_ms << " ms, mean recv delta: "
//					<< best_it->recv_mean_ms << " ms, num probes: "
//					<< best_it->count;
...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacket(int64_t arrival_time_ms,
		size_t payload_size, const RTPHeader& header) {
...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacketInfo(
	...
	if (!uma_recorded_) {
		RTC_HISTOGRAM_ENUMERATION(kBweTypeHistogram,
				BweNames::kReceiverAbsSendTime, BweNames::kBweNamesMax);
		uma_recorded_ = true;
	}
	...
	absl::optional<uint32_t> incoming_bitrate = incoming_bitrate_.Rate(
			arrival_time_ms);
	if (incoming_bitrate) {
	...
	std::vector<uint32_t> ssrcs;
	{
		...
		RTC_DCHECK(inter_arrival_.get());RTC_DCHECK(estimator_.get());
		ssrcs_[ssrc] = now_ms;
		...
		if (payload_size > kMinProbePacketSize
			...
			if (total_probes_received_ < kMaxProbePackets) {
				...
				if (!probes_.empty()) {
					...
					recv_delta_ms = arrival_time_ms
							- probes_.back().recv_time_ms;
				}
//				RTC_LOG(LS_INFO) << "Probe packet received: send time="
//						<< send_time_ms << " ms, recv time=" << arrival_time_ms
//						<< " ms, send delta=" << send_delta_ms
//						<< " ms, recv delta=" << recv_delta_ms << " ms.";
			}
			probes_.push_back(
					Probe(send_time_ms, arrival_time_ms, payload_size));
		...
		if (!update_estimate) {
			...
			if (last_update_ms_ == -1
				...
				absl::optional<uint32_t> incoming_rate = incoming_bitrate_.Rate(
						arrival_time_ms);
				if (incoming_rate
		...
		if (update_estimate) {
			...
			target_bitrate_bps = remote_rate_.Update(&input,
					Timestamp::ms(now_ms)).bps<uint32_t>();
...
bool RemoteBitrateEstimatorAbsSendTime::LatestEstimate(
	...
	// thread.
	RTC_DCHECK(ssrcs);RTC_DCHECK(bitrate_bps);
