...
OveruseEstimator::OveruseEstimator(const OverUseDetectorOptions& options) :
		options_(options), num_of_deltas_(0), slope_(options_.initial_slope), offset_(
				options_.initial_offset), prev_offset_(options_.initial_offset), E_(), process_noise_(), avg_noise_(
				options_.initial_avg_noise), var_noise_(
				options_.initial_var_noise), ts_delta_hist_() {
...
//	BWE_TEST_LOGGING_PLOT(1, "dm_ms", now_ms, t_ts_delta);
	...
	const double Eh[2] = { E_[0][0] * h[0] + E_[0][1] * h[1], E_[1][0] * h[0]
			+ E_[1][1] * h[1] };
...
//	BWE_TEST_LOGGING_PLOT(1, "d_ms", now_ms, slope_ * h[0] - offset_);
	...
	const bool in_stable_state = (current_hypothesis
	...
	const double IKh[2][2] = { { 1.0 - K[0] * h[0], -K[0] * h[1] }, { -K[1]
			* h[0], 1.0 - K[1] * h[1] } };
