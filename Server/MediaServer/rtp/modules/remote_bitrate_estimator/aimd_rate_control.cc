...
double ReadBackoffFactor(const WebRtcKeyValueConfig& key_value_config) {
	std::string experiment_string = key_value_config.Lookup(
			kBweBackOffFactorExperiment);
	double backoff_factor;
	int parsed_values = sscanf(experiment_string.c_str(), "Enabled-%lf",
			&backoff_factor);
	if (parsed_values == 1) {
...
AimdRateControl::AimdRateControl(const WebRtcKeyValueConfig* key_value_config,
		bool send_side) :
		min_configured_bitrate_(congestion_controller::GetMinBitrate()), max_configured_bitrate_(
				DataRate::kbps(30000)), current_bitrate_(
				max_configured_bitrate_), latest_estimated_throughput_(
				current_bitrate_), link_capacity_(), rate_control_state_(
				...
				Timestamp::MinusInfinity()), bitrate_is_initialized_(false), beta_(
				IsEnabled(*key_value_config, kBweBackOffFactorExperiment) ?
						ReadBackoffFactor(*key_value_config) :
						kDefaultBackoffFactor), in_alr_(false), rtt_(
				kDefaultRtt), send_side_(send_side), in_experiment_(
				!AdaptiveThresholdExperimentIsDisabled(*key_value_config)), no_bitrate_increase_in_alr_(
				IsEnabled(*key_value_config,
						"WebRTC-DontIncreaseDelayBasedBweInAlr")), smoothing_experiment_(
				IsEnabled(*key_value_config,
						"WebRTC-Audio-BandwidthSmoothing")), estimate_bounded_backoff_(
				IsEnabled(*key_value_config,
						"WebRTC-Bwe-EstimateBoundedBackoff")), estimate_bounded_increase_(
				IsEnabled(*key_value_config,
						"WebRTC-Bwe-EstimateBoundedIncrease"))/*, initial_backoff_interval_(
				"initial_backoff_interval"), low_throughput_threshold_(
				"low_throughput", DataRate::Zero())*/ {
	// E.g
...
bool AimdRateControl::TimeToReduceFurther(Timestamp at_time,
	...
	const TimeDelta bitrate_reduction_interval = rtt_.Clamped(TimeDelta::ms(10),
			TimeDelta::ms(200));
...
//			|| at_time - time_last_bitrate_decrease_
	...
	if (!bitrate_is_initialized_) {
		...
		if (time_first_throughput_estimate_.IsInfinite()) {
			if (input->estimated_throughput)
				...
				> kInitializationTime && input->estimated_throughput) {
...
double AimdRateControl::GetNearMaxIncreaseRateBpsPerSecond() const {
	...
	double increase_rate_bps_per_second = (avg_packet_size / response_time).bps<
			double>();
...
TimeDelta AimdRateControl::GetExpectedBandwidthPeriod() const {
	...
	double time_to_recover_decrease_seconds = last_decrease_->bps()
...
DataRate AimdRateControl::ChangeBitrate(DataRate new_bitrate,
	...
	DataRate estimated_throughput = input.estimated_throughput.value_or(
	...
	case kRcIncrease:
		...
		if (!(send_side_ && in_alr_ && no_bitrate_increase_in_alr_)) {
			if (link_capacity_.has_estimate()) {
				...
				// increase.
				DataRate additive_increase = AdditiveRateIncrease(at_time,
	...
	case kRcDecrease:
		if (estimated_throughput > DataRate::Zero()/*low_throughput_threshold_*/) {
			...
			new_bitrate = std::min(new_bitrate,
					DataRate::Zero()/*low_throughput_threshold_.Get()*/);
		}
		new_bitrate = std::min(new_bitrate, current_bitrate_);

		if (bitrate_is_initialized_
				&& estimated_throughput < current_bitrate_) {
...
DataRate AimdRateControl::ClampBitrate(DataRate new_bitrate,
	...
	if (!(send_side_ && no_bitrate_increase_in_alr_)) {
		...
		const DataRate max_bitrate = 1.5 * estimated_throughput
				+ DataRate::kbps(10);
...
DataRate AimdRateControl::MultiplicativeRateIncrease(Timestamp at_time,
		Timestamp last_time, DataRate current_bitrate) const {
	double alpha = 1.08;
	if (last_time.IsFinite()) {
		auto time_since_last_update = at_time - last_time;
		alpha = pow(alpha,
				std::min(time_since_last_update.seconds<double>(), 1.0));
	}
...
DataRate AimdRateControl::AdditiveRateIncrease(Timestamp at_time,
	...
	double data_rate_increase_bps = GetNearMaxIncreaseRateBpsPerSecond()
			* time_period_seconds;
