...
#ifndef RTP_INCLUDE_RTP_RTCP_DEFINES_H_
#define RTP_INCLUDE_RTP_RTCP_DEFINES_H_
...
//#include <rtp/api/audio_codecs/audio_format.h>
#include <rtp/api/rtp_headers.h>
#include <rtp/api/transport/network_types.h>
#include <rtp/packet/remote_estimate.h>
...
enum RtxMode {
	kRtxOff = 0x0, kRtxRetransmitted = 0x1, // Only send retransmissions over RTX.
...
struct RTCPReportBlock {
	RTCPReportBlock() :
			sender_ssrc(0), source_ssrc(0), fraction_lost(0), packets_lost(0), extended_highest_sequence_number(
					0), jitter(0), last_sender_report_timestamp(0), delay_since_last_sender_report(
					0) {
	}
	...
	RTCPReportBlock(uint32_t sender_ssrc, uint32_t source_ssrc,
			...
			sender_ssrc(sender_ssrc), source_ssrc(source_ssrc), fraction_lost(
					fraction_lost), packets_lost(packets_lost), extended_highest_sequence_number(
					extended_highest_sequence_number), jitter(jitter), last_sender_report_timestamp(
					last_sender_report_timestamp), delay_since_last_sender_report(
...
struct RtpState {
	RtpState() :
			sequence_number(0), start_timestamp(0), timestamp(0), capture_time_ms(
					-1), last_timestamp_time_ms(-1), media_has_been_sent(false), ssrc_has_acked(
					false) {
	}
...
public:
	...
	virtual void OnReceivedLossNotification(uint32_t ssrc,
			...
			uint16_t seq_num_of_last_received, bool decodability_flag) = 0;
};
...
public:
	...
	virtual void OnReceivedRtcpReceiverReport(
			const ReportBlockList& report_blocks, int64_t rtt,
			int64_t now_ms) = 0;
...
public:
	virtual void RegisterStreamFeedbackObserver(std::vector<uint32_t> ssrcs,
