...
StreamStatisticianImpl::StreamStatisticianImpl(uint32_t ssrc, Clock* clock,
		...
		ssrc_(ssrc), clock_(clock), incoming_bitrate_(
				kStatisticsProcessIntervalMs, RateStatistics::kBpsScale), max_reordering_threshold_(
				max_reordering_threshold), enable_retransmit_detection_(false), jitter_q4_(
				0), cumulative_loss_(0), cumulative_loss_rtcp_offset_(0), last_receive_time_ms_(
				0), last_received_timestamp_(0), received_seq_first_(-1), received_seq_max_(
				-1), last_report_cumulative_loss_(0), last_report_seq_max_(-1) {
}
...
bool StreamStatisticianImpl::UpdateOutOfOrder(const RtpPacketReceived& packet,
		int64_t sequence_number, int64_t now_ms) {
...
void StreamStatisticianImpl::UpdateCounters(const RtpPacketReceived& packet) {
	qpidnetwork::CritScope cs(&stream_lock_);
	...
	int64_t sequence_number = seq_unwrapper_.UnwrapWithoutUpdate(
...
void StreamStatisticianImpl::UpdateJitter(const RtpPacketReceived& packet,
	...
	uint32_t receive_diff_rtp = static_cast<uint32_t>((receive_diff_ms
...
RtcpStatistics StreamStatisticianImpl::CalculateRtcpStatistics() {
	...
	if (exp_since_last > 0 && lost_since_last > 0) {
		...
		stats.fraction_lost = static_cast<uint8_t>(255 * lost_since_last
				/ exp_since_last);
	} else {
...
//	BWE_TEST_LOGGING_PLOT_WITH_SSRC(1, "received_seq_max_pkts",
...
StreamDataCounters StreamStatisticianImpl::GetReceiveStreamDataCounters() const {
	qpidnetwork::CritScope cs(&stream_lock_);
...
bool StreamStatisticianImpl::IsRetransmitOfOldPacket(
		const RtpPacketReceived& packet, int64_t now_ms) const {
...
//	return std::make_unique < ReceiveStatisticsImpl > (clock);
	std::unique_ptr<ReceiveStatisticsImpl> rs(new ReceiveStatisticsImpl(clock));
	return std::move(rs);
}

ReceiveStatisticsImpl::ReceiveStatisticsImpl(Clock* clock) :
		clock_(clock), last_returned_ssrc_(0), max_reordering_threshold_(
				kDefaultMaxReorderingThreshold) {
}

ReceiveStatisticsImpl::~ReceiveStatisticsImpl() {
	qpidnetwork::CritScope cs(&receive_statistics_lock_);
...
void ReceiveStatisticsImpl::SetMaxReorderingThreshold(
	...
	for (auto& statistician : statisticians) {
		statistician.second->SetMaxReorderingThreshold(
				max_reordering_threshold);
	}
}

void ReceiveStatisticsImpl::SetMaxReorderingThreshold(uint32_t ssrc,
		int max_reordering_threshold) {
