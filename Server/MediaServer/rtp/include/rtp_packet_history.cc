...
RtpPacketHistory::StoredPacket::StoredPacket(
		...
		absl::optional<int64_t> send_time_ms, uint64_t insert_order) :
		...
		pending_transmission_(!send_time_ms.has_value()), insert_order_(
				insert_order), times_retransmitted_(0) {
}
...
RtpPacketHistory::RtpPacketHistory(Clock* clock) :
		clock_(clock), number_to_store_(0), mode_(StorageMode::kDisabled), rtt_ms_(
				-1), packets_inserted_(0) {
}
...
// Packet to be inserted behind last packet, expand back.
	...
	RTC_DCHECK_GE(packet_index, 0);RTC_DCHECK_LT(packet_index, packet_history_.size());RTC_DCHECK(packet_history_[packet_index].packet_ == nullptr);

	packet_history_[packet_index] = StoredPacket(std::move(packet),
			send_time_ms, packets_inserted_++);
...
// Packet already resent within too short a time window, ignore.
	...
	std::unique_ptr<RtpPacketToSend> encapsulated_packet = encapsulate(
			*packet->packet_);
...
void RtpPacketHistory::CullOldPackets(int64_t now_ms) {
	int64_t packet_duration_ms = std::max(kMinPacketDurationRtt * rtt_ms_,
...
std::unique_ptr<RtpPacketToSend> RtpPacketHistory::RemovePacket(
	...
	std::unique_ptr<RtpPacketToSend> rtp_packet = std::move(
