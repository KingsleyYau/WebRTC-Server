...
int64_t NtpOffsetMsCalledOnce() {
	...
	int64_t clock_time = mediaserver::TimeMillis();
	int64_t utc_time = mediaserver::TimeUTCMillis();
	return utc_time - clock_time
			+ kNtpJan1970Sec * mediaserver::kNumMillisecsPerSec;
}
...
NtpTime TimeMicrosToNtp(int64_t time_us) {
	...
	// of mediaserver::TimeMillis() clock, the exact offset doesn't matter.
	...
	uint32_t ntp_seconds = static_cast<uint64_t>(time_ntp_us
			/ mediaserver::kNumMicrosecsPerSec);

	// Scale fractions of the second to ntp resolution.
	constexpr int64_t kNtpInSecond = 1LL << 32;
	int64_t us_fractions = time_ntp_us % mediaserver::kNumMicrosecsPerSec;
...
uint32_t SaturatedUsToCompactNtp(int64_t us) {
	...
	if (us <= 0)
		return 0;
	if (us
			>= kMaxCompactNtp * mediaserver::kNumMicrosecsPerSec
					/ kCompactNtpInSecond)
