...
Sdes::Sdes() :
		block_length_(RtcpPacket::kHeaderLength) {
}
...
//						" multiple of 4 bytes";
	...
	const uint8_t* const payload_end = packet.payload()
			+ packet.payload_size_bytes();
...
//			RTC_LOG(LS_WARNING) << "Not enough space left for chunk #"
//					<< (i + 1);
			return false;
		}
...
//				RTC_LOG(LS_WARNING)
//						<< "Unexpected end of packet while reading chunk #"
...
//						<< item_length;
			...
			if (item_type == kCnameTag) {
				if (cname_found) {
//					RTC_LOG(LS_WARNING)
//							<< "Found extra CNAME for same ssrc in chunk #"
//							<< (i + 1);
					return false;
				}
				cname_found = true;
				chunks[i].cname.assign(
						reinterpret_cast<const char*>(looking_at), item_length);
			}
...
//			RTC_LOG(LS_WARNING) << "CNAME not found for ssrc "
//					<< chunks[i].ssrc;
			--number_of_chunks;
			chunks.resize(number_of_chunks);
		}
...
bool Sdes::Create(uint8_t* packet, size_t* index, size_t max_length) const {
	...
	for (const Sdes::Chunk& chunk : chunks_) {
		...
		ByteWriter<uint8_t>::WriteBigEndian(&packet[*index + 5],
				static_cast<uint8_t>(chunk.cname.size()));
