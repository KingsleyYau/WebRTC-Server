...
public:
	...
	void Read(const uint8_t* src, NetworkStateEstimate* target) const {
		int64_t scaled = ByteReader<uint32_t, kFieldValueSize>::ReadBigEndian(
				src);
...
//			RTC_LOG(LS_WARNING) << "Trying to serialize MinusInfinity";
		...
		if (value.IsPlusInfinity()) {
			...
			if (scaled >= kMaxEncoded) {
				scaled = kMaxEncoded;
//				RTC_LOG(LS_WARNING) << ToString(value)
//						<< " is larger than max ("
...
public:
	explicit RemoteEstimateSerializerImpl(
			std::vector<DataRateSerializer> fields) :
			fields_(fields) {
	}
...
const RemoteEstimateSerializer* GetRemoteEstimateSerializer() {
	...
	static auto* serializer = new RemoteEstimateSerializerImpl( { { 1,
			[](E* e) {return &e->link_capacity_lower;} }, { 2,
			[](E* e) {return &e->link_capacity_upper;} }, });
	return serializer;
}

RemoteEstimate::RemoteEstimate() :
		serializer_(GetRemoteEstimateSerializer()) {
	SetSubType(kSubType);
	SetName(kName);
	SetSenderSsrc(0);
}
