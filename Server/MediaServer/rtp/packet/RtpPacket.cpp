...
RtpPacket::RtpPacket(const ExtensionManager* extensions, size_t capacity) :
		extensions_(extensions ? *extensions : ExtensionManager()), buffer_(
				capacity) {
...
bool RtpPacket::ParseBuffer(const uint8_t* buffer, size_t size) {
	if (size < kFixedHeaderSize) {
		LogAync(LOG_WARNING, "RtpPacket::ParseBuffer( "
				"this : %p, "
	...
	if (has_extension_) {
		...
		if (extension_offset + extensions_capacity > size) {
			LogAync(LOG_WARNING, "RtpPacket::ParseBuffer( "
					"this : %p, "
					"[Unsupported rtp extension offset and capacity], "
					"size : %u, "
					"extension_offset : %u, "
					"extensions_capacity : %u "
					")", this, size, extension_offset, extensions_capacity);
			return false;
		}
...
//				LogAync(LOG_DEBUG, "RtpPacket::ParseBuffer( "
//						"this : %p, "
//						"[Extension], "
//						"ssrc : 0x%08x(%u), "
//						"seq : %u, "
...
//						id, length);
	...
	if (payload_offset_ + padding_size_ > size) {
		LogAync(LOG_WARNING, "RtpPacket::ParseBuffer( "
				"this : %p, "
				"[Oversized rtp payload offset], "
				"payload_offset_ : %u "
				"padding_size_ : %u "
				"size : %u "
				")", this, payload_offset_, padding_size_, size);
		return false;
	}
...
//				<< ": expected " << static_cast<int>(extension_entry->length)
//				<< ". received " << length;
		return nullptr;
	}
	if (payload_size_ > 0) {
...
//				<< " after padding was set.";
	...
	const bool two_byte_header_required = id
	...
	if (extensions_size_ > 0) {
		profile_id = ByteReader<uint16_t>::ReadBigEndian(
				data() + extensions_offset - 4);
		if (profile_id == kOneByteExtensionProfileId
				&& two_byte_header_required) {
			...
			size_t expected_new_extensions_size = extensions_size_
					+ extension_entries_.size() + kTwoByteExtensionHeaderLength
					+ length;
...
//								"extension.";
		...
		profile_id =
				two_byte_header_required ?
						kTwoByteExtensionProfileId : kOneByteExtensionProfileId;
	}

	const size_t extension_header_size =
			profile_id == kOneByteExtensionProfileId ?
					kOneByteExtensionHeaderLength :
					kTwoByteExtensionHeaderLength;
	size_t new_extensions_size = extensions_size_ + extension_header_size
			+ length;
...
//				<< "Extension cannot be registered: Not enough space left in buffer.";
	...
	uint16_t extensions_size_padded = SetExtensionLengthMaybeAddZeroPadding(
			extensions_offset);
...
void RtpPacket::PromoteToTwoByteHeaderExtension() {
	...
	RTC_CHECK_GT(extension_entries_.size(), 0);RTC_CHECK_EQ(payload_size_, 0);RTC_CHECK_EQ(kOneByteExtensionProfileId, ByteReader<uint16_t>::ReadBigEndian(
					data() + extensions_offset - 4));
	// Rewrite data.
...
//				<< ", only " << (capacity() - payload_offset_ - payload_size_)
...
//				<< ", packet=" << ToString();
	...
	for (const ExtensionInfo& ext : extension_entries_) {
		if (ext.id == id_to_remove) {
			...
			auto extension_data = new_packet.AllocateRawExtension(ext.id,
					ext.length);
			if (extension_data.size() != ext.length) {
//				RTC_LOG(LS_ERROR) << "Failed to allocate extension id="
//						<< ext.id << ", length=" << ext.length << ", packet="
//						<< ToString();
				return false;
			}
	...
	if (!found_extension) {
//		RTC_LOG(LS_WARNING) << "Extension not present in RTP packet, type="
//				<< type << ", packet=" << ToString();
		return false;
	}
...
std::string RtpPacket::ToString() const {
	...
	result << "{payload_type=" << payload_type_ << "marker=" << marker_
			...
			<< ssrc_ << ", payload_offset=" << payload_offset_
