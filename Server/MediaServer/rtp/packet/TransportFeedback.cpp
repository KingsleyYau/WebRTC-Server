...
constexpr int kBaseScaleFactor = TransportFeedback::kDeltaScaleFactor
		* (1 << 8);
...
uint16_t TransportFeedback::LastChunk::Emit() {
	RTC_CHECK(!CanAdd(0) || !CanAdd(1) || !CanAdd(2));
	if (all_same_) {
...
void TransportFeedback::LastChunk::DecodeOneBit(uint16_t chunk,
		size_t max_size) {
	RTC_CHECK_EQ(chunk & 0xc000, 0x8000);
...
void TransportFeedback::LastChunk::DecodeTwoBit(uint16_t chunk,
		size_t max_size) {
	RTC_CHECK_EQ(chunk & 0xc000, 0xc000);
...
TransportFeedback::TransportFeedback(bool include_timestamps, bool include_lost) :
		include_lost_(include_lost), base_seq_no_(0), num_seq_no_(0), base_time_ticks_(
				0), feedback_seq_(0), include_timestamps_(include_timestamps), last_timestamp_us_(
				0), size_bytes_(kTransportFeedbackHeaderSizeBytes) {
}
...
TransportFeedback::TransportFeedback(TransportFeedback&& other) :
		include_lost_(other.include_lost_), base_seq_no_(other.base_seq_no_), num_seq_no_(
				other.num_seq_no_), base_time_ticks_(other.base_time_ticks_), feedback_seq_(
				other.feedback_seq_), include_timestamps_(
				other.include_timestamps_), last_timestamp_us_(
				other.last_timestamp_us_), received_packets_(
				std::move(other.received_packets_)), all_packets_(
				std::move(other.all_packets_)), encoded_chunks_(
				std::move(other.encoded_chunks_)), last_chunk_(
				other.last_chunk_), size_bytes_(other.size_bytes_) {
	other.Clear();
}
...
bool TransportFeedback::AddReceivedPacket(uint16_t sequence_number,
	...
	if (include_timestamps_) {
		...
		int64_t delta_full = (timestamp_us - last_timestamp_us_)
				% kTimeWrapPeriodUs;
		if (delta_full > kTimeWrapPeriodUs / 2)
			delta_full -= kTimeWrapPeriodUs;
		delta_full +=
				delta_full < 0 ?
						-(kDeltaScaleFactor / 2) : kDeltaScaleFactor / 2;
...
bool TransportFeedback::Parse(const CommonHeader& packet) {
	...
	RTC_CHECK_EQ(delta_sizes.size(), status_count);
...
bool TransportFeedback::Create(uint8_t* packet, size_t* position,
	...
	if (*position + BlockLength() > max_length) {
