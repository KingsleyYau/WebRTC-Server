...
bool Dlrr::Parse(const uint8_t* buffer, uint16_t block_length_32bits) {
	...
	// kReserved = buffer[1];
	RTC_CHECK_EQ(block_length_32bits,
...
//    RTC_LOG(LS_WARNING) << "Invalid size for dlrr block.";
	...
	for (ReceiveTimeInfo& sub_block : sub_blocks_) {
		...
		sub_block.delay_since_last_rr = ByteReader<uint32_t>::ReadBigEndian(
				&read_at[8]);
...
void Dlrr::Create(uint8_t* buffer) const {
	...
	buffer[0] = kBlockType;
	buffer[1] = kReserved;
	ByteWriter<uint16_t>::WriteBigEndian(&buffer[2],
			dchecked_cast<uint16_t>(3 * sub_blocks_.size()));
	// Create sub blocks.
