...
TargetBitrate::BitrateItem::BitrateItem(uint8_t spatial_layer,
		uint8_t temporal_layer, uint32_t target_bitrate_kbps) :
		spatial_layer(spatial_layer), temporal_layer(temporal_layer), target_bitrate_kbps(
				target_bitrate_kbps) {
}
...
void TargetBitrate::Parse(const uint8_t* block, uint16_t block_length) {
	...
	RTC_CHECK_EQ(block[0], kBlockType); RTC_CHECK_EQ(block_length, ByteReader<uint16_t>::ReadBigEndian(&block[2]));
	...
	for (size_t i = 0; i < num_items; ++i) {
		...
		uint32_t bitrate_kbps = ByteReader<uint32_t, 3>::ReadBigEndian(
				&block[index + 1]);
...
void TargetBitrate::AddTargetBitrate(uint8_t spatial_layer,
		uint8_t temporal_layer, uint32_t target_bitrate_kbps) {
	RTC_CHECK_LE(spatial_layer, 0x0F); RTC_CHECK_LE(temporal_layer, 0x0F); RTC_CHECK_LE(target_bitrate_kbps, 0x00FFFFFFU);
	bitrates_.push_back(
...
void TargetBitrate::Create(uint8_t* buffer) const {
	buffer[0] = kBlockType;
	buffer[1] = 0;  // Reserved.
	uint16_t block_length_words = dchecked_cast < uint16_t
			> ((BlockLength() / 4) - 1);
