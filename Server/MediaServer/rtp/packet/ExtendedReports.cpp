...
//				<< "Packet is too small to be an ExtendedReports packet.";
	...
	const uint8_t* const packet_end = packet.payload()
	...
	while (current_block + kBlockHeaderSizeBytes <= packet_end) {
		...
		uint16_t block_length = ByteReader<uint16_t>::ReadBigEndian(
				current_block + 2);
		const uint8_t* next_block = current_block + kBlockHeaderSizeBytes
				+ block_length * 4;
...
//			RTC_LOG(LS_WARNING) << "Unknown extended report block type "
//					<< block_type;
			break;
		}
...
bool ExtendedReports::Create(uint8_t* packet, size_t* index,
