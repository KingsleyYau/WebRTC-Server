...
#ifndef RTP_BASE_BUFFER_H_
#define RTP_BASE_BUFFER_H_
...
public:
	...
	BufferT(BufferT&& buf) :
			size_(buf.size()), capacity_(buf.capacity()), data_(
					std::move(buf.data_)) {
		RTC_DCHECK(IsConsistent());
		buf.OnMovedFrom();
	}
	...
	BufferT(size_t size, size_t capacity) :
			size_(size), capacity_(std::max(size, capacity)), data_(
					capacity_ > 0 ? new T[capacity_] : nullptr) {
		RTC_DCHECK(IsConsistent());
	}
	...
	template<typename U, size_t N, typename std::enable_if<
			internal::BufferCompat<T, U>::value>::type* = nullptr>
	BufferT(U (&array)[N]) :
			BufferT(array, N) {
	}
	...
	template<typename W, typename std::enable_if<
	...
	template<typename U = T, typename F, typename std::enable_if<
			internal::BufferCompat<T, U>::value>::type* = nullptr>
	size_t SetData(size_t max_elements, F&& setter) {
		RTC_DCHECK(IsConsistent());
		const size_t old_size = size_;
		size_ = 0;
		const size_t written = AppendData<U>(max_elements,
				std::forward<F>(setter));
		if (ZeroOnFree && size_ < old_size) {
			ZeroTrailingData(old_size - size_);
		}
		return written;
...
private:
	...
	void ZeroTrailingData(size_t count) {
		RTC_DCHECK(IsConsistent());RTC_DCHECK_LE(count, capacity_ - size_);
