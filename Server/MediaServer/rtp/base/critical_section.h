...
#ifndef RTP_BASE_CRITICAL_SECTION_H_
#define RTP_BASE_CRITICAL_SECTION_H_
...
private:
	const CriticalSection* const cs_;RTC_DISALLOW_COPY_AND_ASSIGN(CritScope)
	;
};
...
public:
	explicit GlobalLockScope(GlobalLock* lock)
			RTC_EXCLUSIVE_LOCK_FUNCTION(lock);
	~GlobalLockScope() RTC_UNLOCK_FUNCTION();

private:
	GlobalLock* const lock_;RTC_DISALLOW_COPY_AND_ASSIGN(GlobalLockScope)
	;
};
