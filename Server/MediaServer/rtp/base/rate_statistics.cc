...
RateStatistics::RateStatistics(int64_t window_size_ms, float scale) :
		buckets_(new Bucket[window_size_ms]()), accumulated_count_(0), num_samples_(
				0), oldest_time_(-window_size_ms), oldest_index_(0), scale_(
				scale), max_window_size_ms_(window_size_ms), current_window_size_ms_(
				max_window_size_ms_) {
}

RateStatistics::RateStatistics(const RateStatistics& other) :
		accumulated_count_(other.accumulated_count_), num_samples_(
				other.num_samples_), oldest_time_(other.oldest_time_), oldest_index_(
				other.oldest_index_), scale_(other.scale_), max_window_size_ms_(
				other.max_window_size_ms_), current_window_size_ms_(
				other.current_window_size_ms_) {
	std::unique_ptr<Bucket[]> buckets(new Bucket[other.max_window_size_ms_]);
...
void RateStatistics::EraseOld(int64_t now_ms) {
	...
	while (num_samples_ > 0 && oldest_time_ < new_oldest_time) {
		...
		RTC_DCHECK_GE(accumulated_count_, oldest_bucket.sum); RTC_DCHECK_GE(num_samples_, oldest_bucket.samples);
