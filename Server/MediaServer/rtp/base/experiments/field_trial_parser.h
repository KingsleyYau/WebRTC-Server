...
#ifndef RTP_BASE_EXPERIMENTS_FIELD_TRIAL_PARSER_H_
#define RTP_BASE_EXPERIMENTS_FIELD_TRIAL_PARSER_H_
...
public:
	FieldTrialConstrained(std::string key, T default_value,
			absl::optional<T> lower_limit, absl::optional<T> upper_limit) :
			FieldTrialParameterInterface(key), value_(default_value), lower_limit_(
					lower_limit), upper_limit_(upper_limit) {
	}
	T Get() const {
		return value_;
	}
...
public:
	FieldTrialEnum(std::string key, T default_value,
			...
			AbstractFieldTrialEnum(key, static_cast<int>(default_value),
					ToIntMap(mapping)) {
	}
	T Get() const {
...
private:
	static std::map<std::string, int> ToIntMap(
			std::map<std::string, T> mapping) {
...
public:
	...
	absl::optional<T> GetOptional() const {
		return value_;
	}
	const T& Value() const {
		return value_.value();
