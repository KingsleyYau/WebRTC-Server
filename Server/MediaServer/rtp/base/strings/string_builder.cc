...
SimpleStringBuilder& SimpleStringBuilder::operator<<(unsigned long i) { // NOLINT
	return AppendFormat("%lu", i);
}

SimpleStringBuilder& SimpleStringBuilder::operator<<(unsigned long long i) { // NOLINT
...
SimpleStringBuilder& SimpleStringBuilder::AppendFormat(const char* fmt, ...) {
	va_list args;
	va_start(args, fmt);
	const int len = std::vsnprintf(&buffer_[size_], buffer_.size() - size_, fmt,
			args);
	if (len >= 0) {
...
//      << "Buffer size was insufficient";
	const size_t chars_added = SafeMin(length, buffer_.size() - size_ - 1);
...
StringBuilder& StringBuilder::AppendFormat(const char* fmt, ...) {
	...
	if (predicted_length > 0) {
		...
		const int actual_length = std::vsnprintf(&str_[size],
				predicted_length + 1, fmt, args);
