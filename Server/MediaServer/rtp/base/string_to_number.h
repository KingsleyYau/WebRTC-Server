...
typename std::enable_if<std::is_integral<T>::value && std::is_signed<T>::value,
		absl::optional<T>>::type StringToNumber(const char* str,
		int base = 10) {
	...
	absl::optional<signed_type> value = string_to_number_internal::ParseSigned(
			str, base);
...
template<typename T>
typename std::enable_if<std::is_floating_point<T>::value, absl::optional<T>>::type StringToNumber(
		const char* str, int base = 10) {
	static_assert(
			std::numeric_limits<T>::max()
