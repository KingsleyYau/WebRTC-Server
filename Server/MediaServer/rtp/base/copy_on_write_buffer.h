...
#ifndef RTP_BASE_COPY_ON_WRITE_BUFFER_H_
#define RTP_BASE_COPY_ON_WRITE_BUFFER_H_
...
#include <rtp/api/scoped_refptr.h>
...
public:
	...
	template<typename T, size_t N, typename std::enable_if<
			internal::BufferCompat<uint8_t, T>::value>::type* = nullptr>
	...
	template<typename T = uint8_t, typename std::enable_if<
			internal::BufferCompat<uint8_t, T>::value>::type* = nullptr>
	const T* data() const {
		return cdata<T>();
	}
	...
	CopyOnWriteBuffer& operator=(const CopyOnWriteBuffer& buf) {
		RTC_DCHECK(IsConsistent());RTC_DCHECK(buf.IsConsistent());
		if (&buf != this) {
			buffer_ = buf.buffer_;
			offset_ = buf.offset_;
			size_ = buf.size_;
	...
	void SetData(const T* data, size_t size) {
		...
		if (!buffer_) {
			buffer_ =
					size > 0 ?
							new RefCountedObject<Buffer>(data, size) : nullptr;
	...
	CopyOnWriteBuffer Slice(size_t offset, size_t length) const {
		...
		RTC_DCHECK_LE(offset, size_);RTC_DCHECK_LE(length + offset, size_);
