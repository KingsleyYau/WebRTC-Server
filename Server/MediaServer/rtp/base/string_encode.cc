...
void hex_encode_with_delimiter(char* buffer, const char* csource, size_t srclen,
		char delimiter) {
	RTC_DCHECK(buffer);
...
size_t hex_decode(char* cbuffer, size_t buflen, const char* source,
		size_t srclen) {
...
size_t hex_decode_with_delimiter(char* cbuffer, size_t buflen,
		const char* source, size_t srclen, char delimiter) {
...
size_t tokenize_with_empty_tokens(const std::string& source, char delimiter,
		std::vector<std::string>* fields) {
	fields->clear();
	size_t last = 0;
	for (size_t i = 0; i < source.length(); ++i) {
