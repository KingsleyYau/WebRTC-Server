...
std::string FindFullName(const std::string& name) {
	...
	while (next_item < trials_string.length()) {
		...
		size_t field_name_end = trials_string.find(kPersistentStringSeparator,
				next_item);
		if (field_name_end == trials_string.npos || field_name_end == next_item)
			break;
		size_t field_value_end = trials_string.find(kPersistentStringSeparator,
				field_name_end + 1);
...
//#ifndef WEBRTC_EXCLUDE_FIELD_TRIAL_DEFAULT
//	if (trials_string) {
//	RTC_DCHECK(FieldTrialsStringIsValid(trials_string)) << "Invalid field trials string:" << trials_string;
//};
//#endif  // WEBRTC_EXCLUDE_FIELD_TRIAL_DEFAULT
