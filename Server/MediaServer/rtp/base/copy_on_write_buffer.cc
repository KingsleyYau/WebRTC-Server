...
CopyOnWriteBuffer::CopyOnWriteBuffer(size_t size) :
		buffer_(size > 0 ? new RefCountedObject<Buffer>(size) : nullptr), offset_(
				0), size_(size) {
	RTC_DCHECK(IsConsistent());
}
...
CopyOnWriteBuffer::CopyOnWriteBuffer(size_t size, size_t capacity) :
		buffer_(
				size > 0 || capacity > 0 ?
						new RefCountedObject<Buffer>(size, capacity) : nullptr), offset_(
				0), size_(size) {
	RTC_DCHECK(IsConsistent());
}
...
bool CopyOnWriteBuffer::operator==(const CopyOnWriteBuffer& buf) const {
	...
	return size_ == buf.size_
			&& (cdata() == buf.cdata()
					|| memcmp(cdata(), buf.cdata(), size_) == 0);
}
