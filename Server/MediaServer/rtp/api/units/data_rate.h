...
#ifndef RTP_API_UNITS_DATA_RATE_H_
#define RTP_API_UNITS_DATA_RATE_H_
...
inline int64_t Microbits(const DataSize& size) {
	constexpr int64_t kMaxBeforeConversion = std::numeric_limits<int64_t>::max()
			/ 8000000;
//	RTC_DCHECK_LE(size.bytes(), kMaxBeforeConversion) << "size is too large to be expressed in microbits";
...
inline int64_t MillibytePerSec(const DataRate& size) {
	constexpr int64_t kMaxBeforeConversion = std::numeric_limits<int64_t>::max()
			/ (1000 / 8);
//	RTC_DCHECK_LE(size.bps(), kMaxBeforeConversion) << "rate is too large to be expressed in microbytes per second";
...
inline Frequency operator/(const DataRate rate, const DataSize size) {
	return Frequency::millihertz(
			data_rate_impl::MillibytePerSec(rate) / size.bytes());
}
inline DataRate operator*(const DataSize size, const Frequency frequency) {
	...
	int64_t millibits_per_second = size.bytes() * 8
			* frequency.millihertz<int64_t>();
