...
bool Parse(const char* buffer, int size, int &packetSize, int &fileSize, int &totalFileSize, FILE *fp) {
	int step = 0;
...
int main(int argc, char *argv[]) {
	...
	FILE *fpresdata = fopen("resdata.mp4", "wb+");
...
//						printf("# Recv byte:%d, recv_total_size:%d, recv_total_data_size:%d \n", received, recv_data_size, recv_total_data_size);
						if ( packet_size > 0 ) {
							fwrite(recv_buffer, 1, received, fpres);
						} else {
							recv_buffer_offset += received;
							bool bParse = Parse(recv_buffer, recv_buffer_offset, packet_size, filedataSize, total_file_size, fpresdata);
							if ( packet_size > 0 ) {
