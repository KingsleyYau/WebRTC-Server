 ...
 * Since: 0.1.5
 */
gint nice_socket_send_messages(NiceSocket *sock, const NiceAddress *to,
 ...
 * Since: 0.1.5
 */
gint nice_socket_send_messages_reliable(NiceSocket *sock, const NiceAddress *to,
		const NiceOutputMessage *messages, guint n_messages) {
	g_return_val_if_fail(sock != NULL, -1);
	g_return_val_if_fail(n_messages == 0 || messages != NULL, -1);
 ...
 * -1 on error. */
gssize nice_socket_recv(NiceSocket *sock, NiceAddress *from, gsize len,
		gchar *buf) {
 ...
 * -1 on error. */
gssize nice_socket_send(NiceSocket *sock, const NiceAddress *to, gsize len,
		const gchar *buf) {
...
gssize nice_socket_send_reliable(NiceSocket *sock, const NiceAddress *to,
...
void nice_socket_free(NiceSocket *sock) {
	 ...
	 */
	nice_debug_verbose("Socket %p(fd %d): Free", sock,
...
static void nice_socket_free_queued_send(NiceSocketQueuedSend *tbs) {
	 ...
	 */
	nice_debug("(fd %d): [TCP/BSD] Send queue buffer free, tbs %p, length %d", tbs->fd, tbs, tbs->length);
	g_free(tbs->buf);
...
void nice_socket_queue_send(GQueue *send_queue, const NiceAddress *to,
	...
	for (i = 0; i < n_messages; i++) {
		 ...
		 * Add by Max 2020/12/04
		 */
		nice_debug("(fd %d): [TCP/BSD] Send queue buffer alloc, tbs %p, length %d", tbs->fd, tbs, tbs->length);
		...
		for (j = 0;
				(message->n_buffers >= 0 && j < (guint) message->n_buffers)
						|| (message->n_buffers < 0
								&& message->buffers[j].buffer != NULL); j++) {
...
void nice_socket_queue_send_with_callback(GQueue *send_queue,
		const NiceOutputMessage *message, gsize message_offset,
		gsize message_len, gboolean head, GSocket *gsock, GSource **io_source,
		GMainContext *context, GSocketSourceFunc cb, gpointer user_data) {
	NiceSocketQueuedSend *tbs;
	guint j;
	gsize offset = 0;
...
//	}
	...
	for (j = 0;
		...
		memcpy(tbs->buf + offset, (guint8 * ) buffer->buffer + message_offset,
				len);
		offset += len;
...
void nice_socket_flush_send_queue(NiceSocket *base_socket, GQueue *send_queue) {
	...
	while ((tbs = g_queue_pop_head(send_queue))) {
		...
		nice_socket_send_reliable(base_socket, to, tbs->length,
				(const gchar *) tbs->buf);
...
void nice_socket_free_send_queue(GQueue *send_queue) {
	g_list_free_full(send_queue->head,
			(GDestroyNotify) nice_socket_free_queued_send);
