...
redo:
    ...
    if (len == AVERROR(EAGAIN) && first_queue_st &&
        ...
        av_log(s, AV_LOG_WARNING,
                "max delay reached. need to consume packet, recv_time:%" PRId64 ", now:%" PRId64 ", seq:%hu, queue_size:%d \n",
				wait_end - s->max_delay,
				av_gettime_relative(),
				ff_rtp_queued_packet_seq(first_queue_st->transport_priv),
				ff_rtp_queued_size(first_queue_st->transport_priv));
