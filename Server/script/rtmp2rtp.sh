...
function Usage {
	echo "Usage : ./rtmp2rtp.sh rtmp://127.0.0.1:1935/live/max rtp://127.0.0.1:12345 1"
}
...
then
	$FFMPEG -probesize 90000 -protocol_whitelist "file,http,https,rtp,rtcp,rtmp,udp,tcp,tls" -thread_queue_size 1024 -i $RTMP_URL \
				-vcodec libx264 -an -payload_type 102 -ssrc 0x12345678 -cname video -preset superfast -profile:v baseline -level 3.0 -g 15 -f rtp "$RTP_URL" \
				-acodec opus -vn -payload_type 111 -ssrc 0x12345679 -cname audio -strict -2 -ac 1 -f rtp "$RTP_URL" > /tmp/webrtc/rtmp2tmp.log 2>&1 &
else
	$FFMPEG -probesize 90000 -protocol_whitelist "file,http,https,rtp,rtcp,rtmp,udp,tcp,tls" -thread_queue_size 1024 -i $RTMP_URL \
