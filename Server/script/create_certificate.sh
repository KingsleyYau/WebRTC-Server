...
openssl req -new -x509 -days 3650 -key output/private/cakey.pem -out output/private/cacert.pem -subj /C=CN/ST=Hongkong/L=Hongkong/O=EST/OU=EST/CN=EST/emailAddress=max.chiu@qpidnetwork.com -extensions v3_ca
# Create User key
openssl genrsa -out output/server.key 2048
# Create User request
openssl req -new -key output/server.key -out output/server.req -subj /C=CN/ST=Hongkong/L=Hongkong/O=mediaserver/OU=mediaserver/CN=mediaserver/emailAddress=kingsleyyau@gmail.com
